...
public partial class ActionSheetPopupContent : UserControl
{
    ...
    {
        var window = Xamarin.Essentials.Platform.MainWindow;
        if (window.Content?.TransformToVisual(Parent as UIElement) is not GeneralTransform transform)
            return;
        ...
        if (WeakParentPopup?.TryGetTarget(out var parent) ?? false)
        {
