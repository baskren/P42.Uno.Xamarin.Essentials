...
public static partial class Platform
{
    ...
    static void CheckAppActions(AndroidIntent intent)
    {
        if (intent?.Action == Intent.ActionAppAction && !intent.GetBooleanExtra(AppActions.extraAppActionHandled, false))
        {
            ...
            if (!string.IsNullOrEmpty(appAction?.Id))
                AppActions.InvokeOnAppAction(CurrentActivity, appAction);
        }
    }
    ...
    internal static bool IsIntentSupported(AndroidIntent intent, string expectedPackageName) =>
        intent.ResolveActivity(AppContext.PackageManager) is { } c && c.PackageName == expectedPackageName;
...
{
    WeakReference<Activity> currentActivity = new(null);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        if (GetIntermediateTask(guid) is { } task)
        {
    ...
    protected override void OnActivityResult(int requestCode, Result resultCode, Intent data)
    {
        ...
        if (GetIntermediateTask(guid, true) is { } task)
        {
