...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        else if (uri.Authority.Equals(UriAuthorityDownloads, StringComparison.OrdinalIgnoreCase))
        {
            ...
            foreach (var prefix in contentUriPrefixes)
            {
                ...
                if (GetDataFilePath(contentUri) is { } filePath)
                    return filePath;
            }
        }
...
#pragma warning disable CS0618 // Type or member is obsolete
                if (contentUri != null && GetDataFilePath(contentUri, $"{MediaStore.MediaColumns.Id}=?", [uriPath]) is { } filePath)
                    return filePath;
#pragma warning restore CS0618 // Type or member is obsolete
            }
        }
    ...
    static string ResolveContentPath(AndroidUri uri)
    {
        ...
        if (GetDataFilePath(uri) is { } filePath)
            return filePath;
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
        var mimeTypes = Platform.ContentResolver.GetStreamTypes(uri, MimeTypes.All);
