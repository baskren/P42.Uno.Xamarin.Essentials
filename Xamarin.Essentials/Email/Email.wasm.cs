...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    {
        ...
        if (message?.Cc != null && message.Cc.Count > 0)
            payload +=
                $"{concatChar()}cc={System.Web.HttpUtility.UrlEncode(string.Join(",", message.Cc), System.Text.Encoding.UTF8)}";
        if (message?.Bcc != null && message.Bcc.Count > 0)
            payload +=
                $"{concatChar()}bcc={System.Web.HttpUtility.UrlEncode(string.Join(",", message.Bcc), System.Text.Encoding.UTF8)}";
        if (!string.IsNullOrWhiteSpace(message?.Subject))
            payload +=
                $"{concatChar()}subject={System.Web.HttpUtility.UrlEncode(message.Subject, System.Text.Encoding.UTF8)}";
        if (!string.IsNullOrWhiteSpace(message?.Body))
            payload +=
                $"{concatChar()}body={System.Web.HttpUtility.UrlEncode(message.Body, System.Text.Encoding.UTF8)}";
