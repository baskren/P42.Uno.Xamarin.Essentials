...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    {
        ...
        var path = Path.Combine(FileSystem.CacheDirectory, Guid.NewGuid().ToString());
...
public partial class FilePickerFileType
{
    ...
    static FilePickerFileType PlatformPngFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.WASM, [FileSystem.Extensions.Png] }
        });
    ...
    static FilePickerFileType PlatformPdfFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.WASM, [FileSystem.Extensions.Pdf] }
        });
}