...
public static partial class FilePicker
{
    ...
    {
        var allowedUtis = options?.FileTypes?.Value?.ToArray() ??
        [
            UTType.Content,
            UTType.Item,
            "public.data"
    ...
    static async Task<string> PlatformExportAsync(byte[] bytes, SaveOptions options)
    {
        ...
        using var exportUrl = new NSUrl(new Uri(tmpPath).AbsoluteUri);
    ...
    static async Task<string> PlatformExportAsync(NSUrl exportUrl, SaveOptions options)
    {
        ...
        using var documentPicker = new UIDocumentPickerViewController([UTType.Folder], UIDocumentPickerMode.Open)
        {
        ...
        if ((await tcs.Task).FirstOrDefault() is { } result)
        {
            ...
            {
                attempt++;
                destFileName = Path.Combine(result.Url.Path,
                    $"{fileName}({attempt}){(string.IsNullOrWhiteSpace(extension) ? null : extension)}");
            }
...
public partial class FilePickerFileType
{
    static FilePickerFileType PlatformImageFileType() =>
        ...
        {
            { DevicePlatform.iOS, [(string)UTType.Image] }
        });

    static FilePickerFileType PlatformPngFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.iOS, [(string)UTType.PNG] }
        });

    static FilePickerFileType PlatformJpegFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.iOS, [(string)UTType.JPEG] }
        });

    static FilePickerFileType PlatformVideoFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.iOS, [UTType.MPEG4, UTType.Video, UTType.AVIMovie, UTType.AppleProtectedMPEG4Video, "mp4", "m4v", "mpg", "mpeg", "mp2", "mov", "avi", "mkv", "flv", "gifv", "qt"
                ]
            }
        });
    ...
    static FilePickerFileType PlatformPdfFileType() =>
        new(new Dictionary<DevicePlatform, IEnumerable<string>>
        {
            { DevicePlatform.iOS, [(string)UTType.PDF] }
        });
}
