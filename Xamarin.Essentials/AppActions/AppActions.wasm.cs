...
public static partial class AppActions
{
    ...
    static AppAction ToAction(this JumpListItem item) =>
        new(ArgumentsToId(item.Arguments), item.DisplayName, item.Description);

    static string ArgumentsToId(string arguments)
    {
        if (arguments?.StartsWith(appActionPrefix) ?? false)
            return Encoding.Default.GetString(Convert.FromBase64String(arguments[appActionPrefix.Length..]));
