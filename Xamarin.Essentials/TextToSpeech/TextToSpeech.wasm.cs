...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    {
        ...
        if (WebAssemblyRuntime.InvokeJS("UnoTextToSpeech_GetVoices()") is { } javascriptResult && !string.IsNullOrWhiteSpace(javascriptResult))
        {
            ...
            foreach (var voice in voices)
            {
                ...
                if (parts.Length > 1)
                {
                    if (parts[1].Split('-') is { } isoLang && isoLang.Length > 1)
                    {
