...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            ...
            if (JavaScripts.Uno.TextToSpeech.GetVoices() is string javascriptResult && !string.IsNullOrWhiteSpace(javascriptResult))
            {
...
#pragma warning restore CC0057 // Unused parameters
        {
            ...
            //var script = $"UnoTextToSpeech_PerformSpeakPromise('{text}', '{name}', '{lang}-{country}', {volume}, {pitch})";
            //await WebAssemblyRuntime.InvokeAsync(script);
            await JavaScripts.Uno.TextToSpeech.SpeakAsync(text, name, $"{lang}-{country}", volume, pitch);
            return;
        }
    }
}
